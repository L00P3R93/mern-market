...
const create = (req, res) => {
    ...
    form.parse(req, async(err, fields, files) => {
        if(err) res.status(400).json({message: "Image could not be uploaded"})
        ...
        } catch (e){return res.status(400).json({error: errorHandler.getErrorMessage(e)})}
    })
}
...
const auctionByID = async(req, res, next, id) => {
    try {
        ...
        req.auction = auction
        next()
    } catch (e) {return res.status('400').json({error: "Could not retrieve"})}
}
...
const defaultPhoto = (req, res) => {return res.sendFile(process.cwd()+defaultImage)}
...
const update = (req, res) => {
    ...
    form.keepExtensions = true
    form.parse(req, async(err, fields, files) => {
        if(err) res.status(400).json({message: "Photo could not be uploaded"})
...
const listOpen = async (req, res) => {
    try {
        let auction = await Auction.find({'bidEnd': {$gt: new Date()}})
                                    .sort('bidStart')
                                    .populate('seller', '_id name')
                                    .populate('bids.bidder', '_id name')
        res.json(auctions)
...
const isSeller = (req, res, next) => {
    ...
    if(!isSeller) return res.status('403').json({error: "User is not authorized"})
    next()
}
