...
export default function Auction({match}){
    ...
    useEffect(() => {
        ...
        read({auctionId: match.params.auctionId}, signal)
            .then((data) => {
                if(data.error) setError(data.error)
                else setAuction(data)
            })
        return function cleanup(){abortController.abort()}
    }, [match.params.auctionId])

    const updateBids = (updatedAuction) => {setAuction(updatedAuction)}
    const update = () => {setJustEnded(true)}
                ...
                <Grid container spacing={6}>
                    <Grid item xs={5} sm={5}>
                        <CardMedia
                            ...
                            title={auction.itemName}
                        />
                        <Typography component="p" variant="subtitle1" className={classes.subheading}>About Item</Typography>
                        <Typography component="p" className={classes.description}>{auction.description}</Typography>
                    </Grid>
                    <Grid item xs={7} sm={7}>
                        {
                            ...
                            ?   (
                                <>
                                    ...
                                    {auction.bids.length > 0 && <Typography component="p" variant="subtitle1" className={classes.lastBid}>{` Last bid: $ ${auction.bids[0].bid}`}</Typography>}
                                    {!auth.isAuthenticated() && <Typography>Please, <Link to='/signin'>sign in</Link> to place your bid.</Typography>}
                                    {auth.isAuthenticated() && <Bidding auction={auction} justEnded={justEnded} updatedBids={updateBids}/>}
                                </>
                                )
