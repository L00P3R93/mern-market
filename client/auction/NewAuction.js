...
export default function NewAuction(){
    ...
    const clickSubmit = () => {
        if(values.bidEnd < values.bidStart){
            ...
            create({userId: jwt.user._id}, {t: jwt.token}, auctionData)
                .then((data) => {
    ...
    if(values.redirect) return (<Redirect to={'/myauctions'}/>)
    return (
        <div>
            <Card className={classes.card}>
                <CardContent>
                    <Typography type="headline" component="h2" className={classes.title}>New Auction</Typography>
                    <br/>
