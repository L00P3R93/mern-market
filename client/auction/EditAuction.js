...
export default function EditShop({match}){
    ...
    const clickSubmit = () => {
        if(auction.bidEnd < auction.bidStart) setError("Auction cannot end before it starts")
        else{
            ...
            upToDate({auctionId: match.params.auctionId},{t: jwt.token}, auctionData)
                .then((data) => {
                    if(data.error) setError(data.error)
    ...
    if(redirect) return (<Redirect to={'/myauctions'}/>)

    return (
        <div className={classes.root}>
            <Card className={classes.card}>
                <CardContent>
                    <Typography type="headline" component={"h2"} className={classes.title}>Edit Auction</Typography>
                    <br/>
                    ...
                    <br/>
                    <TextField id={"name"} label={"Name"} className={classes.textField} value={auction.itemName} onChange={handleChange('itemName')} margin={"normal"}/>
                    <br/>
                    <TextField
