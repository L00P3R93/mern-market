...
export default function EditProduct({match}){
    ...
    useEffect(() => {
        ...
        read({productId: match.params.productId}, signal)
        .then((data) => {
            if(data.error) setValues({...values, error: data.error})
            else setValues({...values, id: data._id, name:data.name, description: data.description, category: data.category, quantity:data.quantity, price: data.price})
        })
    ...
    const clickSubmit = () => {
        ...
        update({shopId: match.params.shopId, productId: match.params.productId},{t: jwt.token}, productData)
        .then((data) => {
            if(data.error) setValues({...values, error: data.error})
    ...
    if(values.redirect) return (<Redirect to={'/seller/shop/edit/' + match.params.shopId} />)

    return (
        <div>
            <Card className={classes.card}>
                <CardContent>
                    <Typography type="headline" component="h2" className={classes.title}>Edit Product</Typography><br/>
                    <Avatar src={imageUrl} className={classes.bigAvatar}/><br/>
                    <input accept="image/*" onChange={handleChange('image')} className={classes.input} id="icon-button-file" type="file" />
                    <label htmlFor="icon-button-file">
                        <Button variant="contained" color="secondary" component="span">Change Image <FileUpload/></Button>
