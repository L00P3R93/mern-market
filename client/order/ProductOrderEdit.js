...
export default function ProductOrderEdit(props){
    ...
    useEffect(() => {
        ...
        getStatusValues(signal).then((data) => {
            ...
            else setValues({...values, statusValues: data, error: ''})
        })
    ...
    const handleStatusChange = productIndex => event => {
        ...
        if(event.target.value === "Cancelled"){
            cancelProduct({shopId: props.shopId, productId: product.product._id}, {t: jwt.token}, {cartItemId: product._id, status: event.target.value, quantity: product.quantity})
                .then((data) => {
            ...
            processCharge({userId: jwt.user._id, shopId: props.shopId, orderId: order._id}, {t: jwt.token}, {cartItemId: product._id, status: event.target.value, amount: (product.quantity * product.product.price)})
                .then((data) => {
                    if(data.error) setValues({...values, error: "Status not updated, try again"})
                    else{
                        props.updateOrders(props.orderIndex, order)
    ...
    return (
        <div>
            <Typography component="span" color="error" className={classes.statusMessage}>{values.error}</Typography>
